...
impl Cash {
    ...
    pub fn symbol(&self) -> String {
        match self {
            ...
            Cash::AlbanianLek => format!(""),                       // ALL  L
            Cash::AlgerianDinar => format!(""),                     // DZD  د.ج
            Cash::AngolanKwanza => format!(""),                     // AOA  Kz
            ...
            Cash::BelarusianRuble => format!(""),                   // BYN  Br
            Cash::BolivianBoliviano => format!(""),                 // BOB  Bs.
            Cash::BotswanaPula => format!(""),                      // BWP  P
            Cash::BrazilianReal => format!(""),                     // BRL  R$
            Cash::BritishPound => format!(""),                      // GBP  £
            Cash::BurmeseKyat => format!(""),                       // MMK  Ks
            Cash::BurundianFranc => format!(""),                    // BIF  Fr
            Cash::CambodianRiel => format!(""),                     // KHR  ៛
            Cash::CanadianDollar => format!(""),                    // CAD  $
            Cash::ChileanPeso => format!(""),                       // CLP  $
            Cash::ChineseYuan => format!(""),                       // CNY  元
            Cash::ColombianPeso => format!(""),                     // COP  $
            Cash::CongoleseFranc => format!(""),                    // CDF  Fr
            Cash::CostaRicanColon => format!(""),                   // CRC  ₡
            Cash::CubanPeso => format!(""),                         // CUP  $
            Cash::CzechKoruna => format!(""),                       // CZK  Kč
            Cash::DominicanPeso => format!(""),                     // DOP  RD$
            Cash::EgyptianPound => format!(""),                     // EGP  ج.م
            Cash::EthiopianBirr => format!(""),                     // ETB  Br
            Cash::Euro => format!(""),                              // EUR  €
            Cash::FijianDollar => format!(""),                      // FJD  $
            Cash::GambianDalasi => format!(""),                     // GMD  D
            Cash::GeorgianLari => format!(""),                      // GEL  ₾
            Cash::GhanaianCedi => format!(""),                      // GHS  ₵
            Cash::GuatemalanQuetzal => format!(""),                 // GTQ  Q
            Cash::GuineanFranc => format!(""),                      // GNF  Fr
            Cash::GuyaneseDollar => format!(""),                    // GYD  $
            Cash::HaitianGourde => format!(""),                     // HTG  G
            Cash::HonduranLempira => format!(""),                   // HNL  L
            Cash::HungarianForint => format!(""),                   // HUF  Ft
            Cash::IcelandicKrona => format!(""),                    // ISK  kr
            Cash::IndianRupee => format!(""),                       // INR  ₹
            Cash::IndonesianRupiah => format!(""),                  // IDR  Rp
            Cash::IranianRial => format!(""),                       // IRR  ﷼
            Cash::IraqiDinar => format!(""),                        // IQD  ع.د
            Cash::IsraeliNewShekel => format!(""),                  // ILS  ₪
            Cash::JamaicanDollar => format!(""),                    // JMD  $
            Cash::JapaneseYen => format!(""),                       // JPY  円
            Cash::KazakhstaniTenge => format!(""),                  // KZT  ₸
            Cash::KenyanShilling => format!(""),                    // KES  Sh
            Cash::KyrgyzstaniSom => format!(""),                    // KGS  c
            Cash::LaoKip => format!(""),                            // LAK  ₭
            Cash::LiberianDollar => format!(""),                    // LRD  $
            Cash::LibyanDinar => format!(""),                       // LYD  ل.د
            Cash::MacedonianDenar => format!(""),                   // MKD  ден
            Cash::MalagasyAriary => format!(""),                    // MGA  Ar
            Cash::MalawianKwacha => format!(""),                    // MWK  MK
            Cash::MalaysianRinggit => format!(""),                  // MYR  RM
            Cash::MauritanianOuguiya => format!(""),                // MRU  UM
            Cash::MauritianRupee => format!(""),                    // MUR  Rs
            Cash::MexicanPeso => format!(""),                       // MXN  $
            Cash::MoldovanLeu => format!(""),                       // MDL  L
            Cash::MongolianTogrog => format!(""),                   // MNT  ₮
            Cash::MozambicanMetical => format!(""),                 // MZN  MT
            Cash::NewTaiwanDollar => format!(""),                   // TWD  $
            Cash::NewZealandDollar => format!(""),                  // NZD  $
            Cash::NicaraguanCordoba => format!(""),                 // NIO  C$
            Cash::NigerianNaira => format!(""),                     // NGN  ₦
            Cash::NorthKoreanWon => format!(""),                    // KPW  ₩
            Cash::NorwegianKrone => format!(""),                    // NOK  kr
            Cash::PakistaniRupee => format!(""),                    // PKR  Rs
            Cash::PapuaNewGuineanKina => format!(""),               // PGK  K
            Cash::ParaguayanGuarani => format!(""),                 // PYG  ₲
            Cash::PeruvianSol => format!(""),                       // PEN  S/.
            Cash::PhilippinePeso => format!(""),                    // PHP  ₱
            Cash::PolishZloty => format!(""),                       // PLN  zł
            Cash::RomanianLeu => format!(""),                       // RON  lei
            Cash::RwandanFranc => format!(""),                      // RWF  Fr
            Cash::RussianRuble => format!(""),                      // RUB  ₽
            Cash::SamoanTala => format!(""),                        // WST  T
            Cash::SerbianDinar => format!(""),                      // RSD  дин.
            Cash::SeychelloisRupee => format!(""),                  // SCR  Rs
            Cash::SierraLeoneanLeone => format!(""),                // SLL  Le
            Cash::SingaporeDollar => format!(""),                   // SGD  $
            Cash::SolomonIslandsDollar => format!(""),              // SBD  $
            Cash::SomaliShilling => format!(""),                    // SOS  Sh
            Cash::SomalilandShilling => format!(""),                // SLS  Sl
            Cash::SouthAfricanRand => format!(""),                  // ZAR  R
            Cash::SouthKoreanWon => format!(""),                    // KRW  ₩
            Cash::SouthSudanesePound => format!(""),                // SSP  £
            Cash::SriLankanRupee => format!(""),                    // LKR  ரூ
            Cash::SudanesePound => format!(""),                     // SDG  ج.س.
            Cash::SurinameseDollar => format!(""),                  // SRD  $
            Cash::SwedishKrona => format!(""),                      // SEK  kr
            Cash::SwissFranc => format!(""),                        // CHF  Fr.
            Cash::SyrianPound => format!(""),                       // SYP  ل.س
            Cash::TajikistaniSomoni => format!(""),                 // TJS  с.
            Cash::TanzanianShilling => format!(""),                 // TZS  Sh
            Cash::ThaiBaht => format!(""),                          // THB  ฿
            Cash::TonganPaAnga => format!(""),                      // TOP  T$
            Cash::TransnistrianRuble => format!(""),                // PRB  p.
            Cash::TrinidadandTobagoDollar => format!(""),           // TTD  $
            Cash::TunisianDinar => format!(""),                     // TND  د.ت
            Cash::TurkishLira => format!(""),                       // TRY  ₺
            Cash::TurkmenistanManat => format!(""),                 // TMT  m.
            Cash::UgandanShilling => format!(""),                   // UGX  USh
            Cash::UkrainianHryvnia => format!(""),                  // UAH  ₴
            Cash::UnitedArabEmiratesDirham => format!(""),          // AED د.إ
            Cash::UnitedStatesDollar => format!(""),                // USD  $
            Cash::UruguayanPeso => format!(""),                     // UYU  $
            Cash::UzbekistaniSom => format!(""),                    // UZS  Sʻ
            Cash::VanuatuVatu => format!(""),                       // VUV  Vt
            Cash::VenezuelanBolivarSoberano => format!(""),         // VES Bs.
            Cash::VietnameseDong => format!(""),                    // VND  ₫
            Cash::YemeniRial => format!(""),                        // YER  ﷼
            Cash::ZambianKwacha => format!(""),                     // ZMW  ZK
            Cash::RTGSdollar => format!(""),                        // ZWB  RTGS$
        }
    }
